...
class APIManager {
  Future signIn(String email, String password, [String baseUrl]) async {
    ...
    try {
      var response = await http.post('${Strings.baseUrl}/api/login',
          headers: {}, body: data);
  ...
  Future signUp(String email, String password) async {
    ...
    dynamic responseJson = {};
    try {
      var response = await http.post('${Strings.baseUrl}/api/register',
          headers: {}, body: data);
      print('API Manger : ${response.statusCode}');
      if (response.statusCode == 200) {
        responseJson = jsonDecode(response.body);
        ...
        responseJson = {'status': 400, 'message': 'Signup Failed'};
      } else {
        responseJson = {'status': 500, 'message': 'Something went wrong.'};
      }
      return responseJson;
